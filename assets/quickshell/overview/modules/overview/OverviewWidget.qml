...
Item {
    ...
    readonly property string screenName: panelWindow.screen?.name ?? ""
    ...
    readonly property int workspaceGroup: Math.floor((root.activeWorkspaceId - 1) / workspacesShown)
    property bool monitorIsFocused: (HyprlandData.monitors.find(m => m.focused)?.name ?? Hyprland.focusedMonitor?.name ?? "") === screenName
    ...
    property real workspaceImplicitWidth: (monitorData?.transform % 2 === 1) ? 
        ((monitorData?.height / monitorData?.scale - (monitorData?.reserved?.[0] ?? 0) - (monitorData?.reserved?.[2] ?? 0)) * root.scale) :
        ((monitorData?.width / monitorData?.scale - (monitorData?.reserved?.[0] ?? 0) - (monitorData?.reserved?.[2] ?? 0)) * root.scale)
    property real workspaceImplicitHeight: (monitorData?.transform % 2 === 1) ? 
        ((monitorData?.width / monitorData?.scale - (monitorData?.reserved?.[1] ?? 0) - (monitorData?.reserved?.[3] ?? 0)) * root.scale) :
        ((monitorData?.height / monitorData?.scale - (monitorData?.reserved?.[1] ?? 0) - (monitorData?.reserved?.[3] ?? 0)) * root.scale)

    property real workspaceNumberMargin: 80
    property real workspaceNumberSize: Config.options.overview.workspaceNumberBaseSize * (monitorData?.scale ?? 1)
    property int workspaceZ: 0
    property int windowZ: 1
    property int windowDraggingZ: 99999
    property real workspaceSpacing: Config.options.overview.workspaceSpacing
    property color overviewPanelColor: ColorUtils.applyAlpha(
        ColorUtils.mix(Appearance.colors.colLayer0, Appearance.colors.colOnLayer0, 0.88),
        0.92
    )
    property color overviewPanelBorderColor: ColorUtils.applyAlpha(
        ColorUtils.mix(Appearance.colors.colLayer0Border, Appearance.colors.colOnLayer0, 0.82),
        0.75
    )
    property color workspaceBaseColor: ColorUtils.applyAlpha(
        ColorUtils.mix(Appearance.colors.colLayer1, Appearance.colors.colLayer0, 0.40),
        0.92
    )
    property color workspaceHoverColor: ColorUtils.applyAlpha(
        ColorUtils.mix(workspaceBaseColor, Appearance.colors.colLayer1Hover, 0.72),
        0.96
    )
    ...
    property var rowsWithContent: {
        ...
        const currentWorkspace = root.activeWorkspaceId;
    ...
    Rectangle {
        ...
        color: root.overviewPanelColor
        border.width: 1
        border.color: root.overviewPanelBorderColor
                ...
                delegate: RowLayout {
                    ...
                    Repeater {
                        ...
                        Rectangle {
                            ...
                            property color defaultWorkspaceColor: root.workspaceBaseColor
                            property color hoveredWorkspaceColor: root.workspaceHoverColor
        ...
        Item {
            ...
            Repeater {
                ...
                delegate: OverviewWindow {
                    ...
                    widgetMonitorId: root.monitorData?.id ?? -1
